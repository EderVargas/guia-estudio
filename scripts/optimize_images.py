...
def optimize_image(input_path, output_folder, config):
    ...
    try:
        ...
        for format_name, size in results.items():
            ...
            print(f"  ✅ {format_name}: {size:.2f} KB (reducción: {reduction:.1f}%)")
...
def main():
    """Función principal"""
    print("\n" + "=" * 60)
    print("🖼️  OPTIMIZADOR DE IMÁGENES PARA CUESTIONARIOS")
    print("=" * 60 + "\n")
    ...
    if not image_files:
        print(f"❌ No se encontraron imágenes en: {CONFIG['input_folder']}")
        print(f"\n💡 Coloca tus imágenes en la carpeta '{CONFIG['input_folder']}' y vuelve a ejecutar.")
        return
